...
export class WindsController {
  ...
  getSingleWind(@Param('altitude') altitude: number, @Param('lat') lat: number, @Param('lon') lon: number, @Param('datetime') dt: string) {
      return this.windsService.getSingleWind(altitude, lat, lon, dt);
  }
}
